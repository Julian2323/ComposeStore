package com.example.composestore.presentation.auth.login
...
fun LoginScreen(
    ...
    val authResource = viewModel?.loginFlow?.collectAsState()
        Column(
            ...
            Text(
                text = "Login to view our amazing options!",
            ...
            Spacer(modifier = Modifier.height(16.dp))
            Button(onClick = { viewModel.loginUser(email, password) }, modifier = Modifier.fillMaxWidth()) {
        ...
        authResource?.value?.let {
            ...
            when (it) {
                ...
                is Resource.Loading -> {
                    CircularProgressIndicator(modifier = Modifier.padding(horizontal = 150.dp, vertical = 300.dp))
                }
                is Resource.Success -> {
                    LaunchedEffect(Unit) {
